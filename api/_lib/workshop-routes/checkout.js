import { json, readJson, supabase } from '../ebook.js'
import { WORKSHOP_RETURN_PATH, WORKSHOP_SLUG, accessHash, autoRedirectInvoiceUrl, checkoutReturnUrl, cleanEmail, cleanText, createRegistrationIdentity, digits, getEdition, validCpf, validEmail } from '../workshop.js'
...
export default async function handler(request, response) {
  ...
  try {
    ...
    const paymentPayload = {
      ...
      callback: { successUrl: checkoutReturnUrl(id, token), autoRedirect: true },
    }
    const paymentResponse = await fetch(`${apiUrl}/payments`, { method: 'POST', headers, body: JSON.stringify(paymentPayload) })
    ...
    return json(response, 201, { checkoutUrl: autoRedirectInvoiceUrl(payment.invoiceUrl), confirmationUrl: `${WORKSHOP_RETURN_PATH}?inscricao=${encodeURIComponent(id)}&token=${encodeURIComponent(token)}` })
  } catch (error) {
