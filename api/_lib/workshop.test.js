...
import { autoRedirectInvoiceUrl, checkoutReturnUrl, createIcs, googleCalendarUrl, validCpf } from './workshop.js'
...
describe('workshop registration helpers', () => {
  ...
  it('uses the public workshop domain and forces the invoice return', () => {
    expect(checkoutReturnUrl('registration-id', 'secret-token')).toBe('https://www.radarpraxia.com/lp/workshop-ia-2026/inscricoes/confirmacao?inscricao=registration-id&token=secret-token')
    expect(autoRedirectInvoiceUrl('https://www.asaas.com/i/abc')).toBe('https://www.asaas.com/i/abc?autoRedirect=true')
  })
})
